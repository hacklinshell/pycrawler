...
const bodyXmlParser = require('koa-xml-body') //解析xml body请求
...
const databases = new Dstabase(Modules.getModules());
...
_each(_.key(m), function(v, k) {
    app.use(mount('/' + v, m[v].routers()));
});

app.use(mount('/', function*() {
    this.body = 'project init'
}));
databases.initialize(function(err, models) {
    if (err) {
        throw err;
    }
    app.models = models;
